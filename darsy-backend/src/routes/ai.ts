    ...
    for (const fetchUrl of candidates) {
        try {
            ...
            const response = await fetch(fetchUrl, {
                headers: {
                    'User-Agent': 'Mozilla/5.0 (compatible; DarsyBot/1.0)',
            ...
            const text = data.text.slice(0, 5000).trim();
...
router.post('/explain', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        ...
        // Regeneration limit: free users get 1 regen/day
        if (forceRegenerate && !isPremium) {
            const existingAnswer = await AiAnswer.findOne({ docId, language });
            if (existingAnswer) {
                const todayStart = new Date();
                todayStart.setHours(0, 0, 0, 0);
                // Check if this user has already regenerated today by checking updatedAt
                // We use a separate daily regen tracker on user
                const lastRegen = user.aiLastRegenDate;
                if (lastRegen && new Date(lastRegen) >= todayStart) {
                    return res.status(429).json({ error: 'You have used your 1 free regeneration for today. Upgrade to Pro for unlimited regenerations.' });
                }
                await User.findByIdAndUpdate(req.userId, { aiLastRegenDate: new Date() });
            }
        }
