...
const THEMES = {
    white: { bgColor: '#ffffff', headlineColor: '#3aaa6a', udarsyColor: '#111111' },
    green: { bgColor: '#3aaa6a', headlineColor: '#ffffff', udarsyColor: '#ffffff' },
};
...
const ContentCreator = () => {
    ...
    // Step 3 — posters stored as { url, theme } so UDARSY color is always correct
    ...
    const generateContent = async () => {
        ...
        try {
            const response = await makeLLMRequest([
                {
                    role: 'system',
                    content: `You are an expert creative director for Udarsy, an educational platform for Moroccan students.
        ...
        try {
            ...
            const [posterData, captionText] = await Promise.all([
                ...
                makeLLMRequest([
                    {
                        role: 'system',
                        content: `You are a social media expert for Udarsy, an educational platform for Moroccan students.
Write one engaging social media caption that works across Instagram, Facebook and TikTok.
- Use relevant emojis
- Include 3–5 relevant hashtags at the end
- Mention Udarsy (e.g. "— Udarsy Team 🌿")
- Write in ${language}
...
- Return ONLY the caption text, no extra explanation`,
    ...
    // ── Stamp UDARSY + optional logo onto canvas ──────────────────────────────
    const applyLogo = async (entry = selectedPoster, udarsyOn = showUdarsy, logoOn = showLogo, corner = logoCorner) => {
        ...
        try {
            ...
            if (udarsyOn) {
                const udarsyPad = canvas.width * 0.03;
                const udarsySize = Math.round(canvas.width * 0.038);
                ctx.font = `700 ${udarsySize}px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`;
                ctx.textBaseline = 'top';
                const udarsyW = ctx.measureText('UDARSY').width;
                ctx.fillStyle = poster.theme === 'green' ? '#ffffff' : '#111111';
                ctx.fillText('UDARSY', canvas.width - udarsyW - udarsyPad, udarsyPad);
            }
    ...
    const handleSelectPoster = (entry) => {
        ...
        applyLogo(entry, showUdarsy, showLogo, logoCorner);
    };
                    ...
                    <div className="cc-poster-layout">
                        ...
                        <div className="cc-logo-panel">
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                                    {CORNERS.map(c => (
                                        <button
                                            ...
                                            onClick={() => { setLogoCorner(c.id); applyLogo(selectedPoster, showUdarsy, showLogo, c.id); }}
                                        >
                                            {c.label}
                                        </button>
                                    ))}
                                ...
                                <div style={{ display: 'flex', gap: 16, fontSize: '0.85rem' }}>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                                        <input type="checkbox" checked={showLogo} onChange={e => {
                                            const v = e.target.checked; setShowLogo(v); applyLogo(selectedPoster, showUdarsy, v, logoCorner);
                                        }} /> Show Logo
                                    </label>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                                        <input type="checkbox" checked={showUdarsy} onChange={e => {
                                            const v = e.target.checked; setShowUdarsy(v); applyLogo(selectedPoster, v, showLogo, logoCorner);
                                        }} /> Show UDARSY Text
                                    </label>
                                </div>
                            </div>
                            ...
                            <button
                                className="pg-btn pg-btn-primary"
                                style={{ width: '100%' }}
                                onClick={() => download(compositedUrl || selectedPoster?.url, `udarsy-${Date.now()}.png`)}
                                disabled={!compositedUrl || loading}
                            >
