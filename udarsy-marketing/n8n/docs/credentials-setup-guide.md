...
2. Send: `/newbot`
3. Enter a name: `Udarsy Marketing Bot`
4. Enter a username: `udarsy_marketing_bot` (must end in "bot")
...
3. Name: `Udarsy n8n` → Click **Create**
...
3. Name it: `Udarsy Marketing Automation`
...
4. App name: `Udarsy Social` → Create app
...
2. Top right: Select your app (`Udarsy Social`)
...
3. Add your sender email (e.g., `noreply@udarsy.app`)
...
Your MongoDB is already running (Udarsy backend uses it). The connection string is in:
```
udarsy-backend/src/config/database.ts
```
or in your `.env` file — look for `MONGODB_URI` or `MONGO_URL`.

It looks like:
```
mongodb://localhost:27017/udarsy
```
or
```
mongodb+srv://username:password@cluster.mongodb.net/udarsy
```

### Step 2 — Important: Docker networking
Since n8n runs in Docker and MongoDB might run on your host machine, use:
```
mongodb://host.docker.internal:27017/udarsy
```
...
4. Fill in:
   ```
   Connection String: mongodb://host.docker.internal:27017/udarsy
   Database: udarsy
   ```
5. Click **Save**
6. Name it: `MongoDB Udarsy`
